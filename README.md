...
# Extra payment return domains (comma-separated), e.g. staging hosts.
# The production domains littlereads.com.ng and www.littlereads.com.ng are
# already pinned IN CODE (src/lib/checkout-callback.ts) and always win over
# NEXT_PUBLIC_SITE_URL as the callback fallback — this var only EXTENDS the
# trusted list.
# Example: ALLOWED_CALLBACK_HOSTS=staging.littlereads.com.ng
...
2. **Return reconcile** — the customer is redirected back to
   `/checkout/success?ref=…`, which re-verifies server-side. The callback
   origin comes from the request host when it is trusted (the production
   domains are pinned in code; `*.vercel.app` and loopback are always
   allowed) and otherwise falls back to the pinned production domain —
   never to a foreign host, no matter what `NEXT_PUBLIC_SITE_URL` says.
